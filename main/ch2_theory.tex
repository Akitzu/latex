...
\end{equation*}
by the jacobian $\dpmap^k\vert_{\z^\star}$ and especially by its eigenvalues $\lambda_1$ and $\lambda_2$. Using \equref{eq:pmap-det}, the determinant of $\text{det}(\dpmap^k\vert_{\z^\star})~=~1$ which is equivalent as saying that $\dpmap^k(\z^\star)$ is an element of $\text{SL}(2,\mathbb{R})$. Depending on the value of $\text{Tr}(\dpmap^k\vert_{\z^\star})$, the eigenvalues are either~:
