...
\end{equation*}
Following the trajectory of the homoclinic points $h_0$ and $m_0$ and tracing them enough time backward allows to reduce and at some point forget about the contribution of $\gamma_1$ to the flux. Since the flux through $\Sl$ and $\pmap^k(\Sl)$ is equal (again, flux conservation of $\pmap$), the same idea as for $\gamma_3$ can be applied to $\gamma_1$. Applying the map repetitively will make the length of $\pmap^k(\gamma_1)$ and thus its contribution to the flux calculation tend to zero. Including these considerations in \equref{eq:contour-2} and taking the the number of forward and backward iterations to infinity gives~:
...
\subsection{Effect of the perturbation amplitude on the turnstile flux}
