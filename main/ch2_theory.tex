...
In fact the flux through the surface $\Sl$ is the turnstile area that we want to calculate and using stokes theorem we obtain that
\begin{equation}
...
\end{equation}
Now the tricky part is to see that by taking the $\gamma_0$ is joining two homoclinics and tracing it backward to $\pmap^{-1}(\gamma_0)$ the surface create by taking this evolution $\Se$ is not adding contribution to the flux calculation as it was obtained by following the magnetic field $\textbf{B}$. it can therefore be written that eq.. becomes~:
\begin{equation}
    \Phi_{lobe} = \int_{\Sl+\Se} \textbf{B}\cdot\textbf{dS} = \int_{\Gamma} \textbf{A}\cdot \textbf{dl}
\end{equation}
where $\Gamma = \gamma_1 + \gamma_2 + \gamma_3 + \gamma_4$ with $\gamma_3 = \pmap^{-1}(\gamma_0)$. 

Insert \figref{fig:flat-tangle} shows a flat version of a manifold in a 2d $\phi$ section. 

we see that the backward evolution takes the curve $\gamma_0$ to $\gamma_3$ and this can be used by repetively applying the to make the $\gamma_0$ curve go towards the $x_u^\star$ fixed point.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{images/turnstile/line_tangle.png}
    \vspace{10px}
    \caption{Flat version of the manifold tangle structure. 
    The forward evolution $\pmap$ maps point from the $x_u^\star$ fixed point in the direction of $x_s^\star$. The backward evolution does the converse. Here there are two hetero/homo-clinics denoted by the squared $s$ and the $circ$. The "closing curves" $\gamma_1$ and $\gamma_3$ are explicitly enhance.
    }
    \label{fig:flat-tangle}
\end{figure}

Now the flux through the lobe is also the same due to the flux conservation of $\pmap$ as the one through $\pmap^k(\Sigma)$ and one can take the same reasoning to make the $\gamma_1$ curve as small as possible.
...
if we write $m^t$ and $h^t$ the two homoclinics intersections we see how they are related to the case of the standard map.


in the case of and that was the idea however it was also found to not work well due to the fact that the exponential behaviour near the fixed point cause the number of iteration to be limited.
A better way to consider the closing integrals are by stopping in the linear regime describe in [sec:manifold]. We know that there the curves $\gamma_1$ near $x$ and $\gamma_3$ near $x$ are in fact segments and we can estimate by discretizing the integral have a.
...
\begin{figure}[H]
    ...
    \begin{minipage}{0.5\textwidth} % Adjust width as needed
        ...
        \begin{subfigure}[b]{\textwidth}
            \centering
            \includegraphics[width=\textwidth]{images/turnstile/turnstile_poincare_3.png}
            \caption{}
            \label{fig:}
        \end{subfigure}
    \end{minipage}
    ...
    \begin{minipage}{0.45\textwidth} % Adjust width as needed
        ...
        \begin{subfigure}[b]{\textwidth}
            \centering
            \includegraphics[width=\textwidth]{images/turnstile/coutour_fwd_5.png}
            \caption{}
            \label{fig:}
        \end{subfigure}
        \vfill
        ...
        \begin{subfigure}[b]{0.99\textwidth}
            \centering
            \includegraphics[width=\textwidth]{images/turnstile/coutour_bwd_1.png}
            \caption{}
            \label{}
        \end{subfigure}
    \end{minipage}%
...
in the case of an island we have first that the need to goes $\pmap^m$ to come back on the same part of the poincare section. it means for example in the case of the high aspect ratio perturbed with a $n/m = 2/3$ perturbation that
...
we also see here that there are in fact two manifold structure, one joining the $x_1^\star$ and $x_2^\star$ by the inside and the othere joining them by the outside.