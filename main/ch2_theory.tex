...
\begin{figure}[h!]
    \subfloat[W7-X]{\label{fig:w7x-default}
        \includegraphics[width=0.3\textwidth]{images/theory/default.pdf}
    }
    \hfill
    \subfloat[NCSX]{%
...
For instance in figure \figref{fig:w7x-default} we see the 
...
\end{equation*}
by the jacobian $\dpmap^k\vert_{\z^\star}$ and especially by its eigenvalues $\lambda_1$ and $\lambda_2$. Using \equref{eq:pmap-det}, the determinant of $\text{det}(\dpmap^k\vert_{\z^\star})~=~1$ which is equivalent as saying that $\dpmap^k(\z^\star)$ is an element of $\text{SL}(2,\mathbb{R})$. 


Depending on the value of $\text{Tr}(\dpmap^k\vert_{\z^\star})$ or equivalently on the value of the Greenes residue $\mathcal{R} = \frac{1}{2} - \frac{1}{4}\text{Tr}(\dpmap^k)$, the eigenvalues are either~:
